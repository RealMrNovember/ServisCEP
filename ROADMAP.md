...
| **B10** | Senkronizasyon motoru — backend yarısı | Version tabanlı optimistic concurrency, sync_conflicts, idempotent client-ID create, Customer geri dönüşüm kutusu (Phase 16-17) | ✅ Backend yarısı tamamlandı ve doğrulandı (99/99 test yeşil + production smoke test) — **mobil senkron motoru (Drift outbox + SyncService) hâlâ sırada**, bkz. detaylar aşağıda |
...
- **Pending — Mobil Senkron Motoru** (kullanıcı kararı: *"Backend + mobil sync motoru birlikte"*): `AuthRepository`'ye gerçek backend token auth, Drift şema göçü (v4) ile sync-queue/outbox tablosu + `version`/`updatedAt` alanları, yeni bir `SyncService` (connectivity_plus'ı dinleyip Dio ile kuyruğu boşaltan), en az Customer+Job repository'lerinin create/update akışlarının kuyruğa yazması, 409 çakışma yanıtının sessizce yeniden denenmek yerine kullanıcıya "gözden geçir" olarak işaretlenmesi, ve `CustomerLedgerEntries`'in sunucudan **çekilen** (push edilmeyen) bir kavram olarak ele alınması — mobil zaten `JobsRepository.completeWithPrice` içinde kendi cari kaydını yerel olarak oluşturuyor, bunun backend'in otomatik cari mantığıyla çakışıp iki kez oluşmaması gerekiyor.
...
| **Hesap Yönetimi (Web Panel)** | Kayıt olan kullanıcılar e-posta adreslerini göremiyor, parolalarını değiştiremiyor, profil fotoğrafı ekleyemiyor — "Hesabım" ekranı eksik | W3 (Web Panel) genişletmesi | Filament'in hazır `EditProfile` sayfası tenant panele (`/panel`) bağlanacak: e-posta görüntüleme + değiştirme (doğrulama akışıyla), parola değiştirme, avatar/profil fotoğrafı yükleme |
