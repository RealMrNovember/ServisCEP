...
| **Belge Şablonları (Teklif/Proforma Tasarımları)** | Kullanıcı belgesini birkaç hazır tasarımdan biriyle üretebilmeli ve birini **varsayılan** seçip her yeni belgede onu kullanabilmeli. Seçim belge bazında da değiştirilebilir (tek bir müşteriye farklı bir tasarım göndermek isteyebilir). | V2 — Phase 13 (PDF Engine) genişletmesi. Kullanıcı isteği (2026-08-27). | ⬜ Planlandı — tasarım hazır: [docs/21 — Belge Şablonları](docs/21-belge-sablonlari.md). **Şablon KOD değil VERİ:** Belge üreticisi bugün yalnızca mobilde (Flutter `pdf` paketi); web panelde PDF üretimi hiç yok (bkz. Sıradaki İşler → W3). Şablonlar her motora ayrı ayrı kodlanırsa aynı teklif iki yerde iki farklı görünümde çıkar — proje bu hatayı abonelik süresi ve belge toplamı hesaplarında zaten iki kez yaşadı. Şablon; yerleşim, tipografi ölçeği, vurgu rengi ve bölüm sırasını tanımlayan bir **veri yapısı** olarak tutulmalı, iki motor da onu okumalı. **Çevrimdışı kısıtı:** şablonlar uygulamayla birlikte GELMELİ. Sunucudan indirilen bir şablon, internetsiz belge üretimini kırar — uygulamanın temel vaadi bu. Sunucu yalnızca *hangi şablonun varsayılan olduğunu* taşır (`companies.default_template`), şablonun kendisini değil. **Sürüm uyumu:** varsayılan olarak seçilen şablon, eski bir uygulama sürümünde bulunmayabilir; bilinmeyen şablon adı sessizce klasik tasarıma düşmeli, belge üretimi ASLA hata vermemeli. **Paket kademesi fırsatı:** ücretsiz pakette tek şablon, ücretlide tamamı — altbilgi kaldırma ile aynı mantık (bkz. Sıradaki İşler → Belge). |
...
- [x] **İç test kanalında 0.2.12 duruyordu** (2026-08-29) — Play, bir
      kullanıcıyı uygun olduğu EN ÖNCELİKLİ kanaldan besliyor ve iç test
      kapalı testin önünde. İç test listesindeki hesaplar, kapalı teste
      de katılmış olsalar bile aylar öncesinin 0.2.12'sini (build 14)
      alıyorlardı. Yayın hattı artık aynı AAB'yi tek yüklemede iki
      kanala birden atıyor (`tracks: internal,alpha`) ve mevcut sürüm
      `play_sync_tracks.py` ile iç teste taşındı.
- [ ] **İç test kullanıcıları API'den yönetilemiyor** — Test grubunu iç
      test kanalına da eklemek denendi, Play 403 döndü: *"The internal
      track has been upgraded to use open or closed testing; switch back
      to communities-based testing before using the API for this
      track."* İç test kullanıcıları yalnızca Console'dan, e-posta
      listesiyle yönetilebiliyor. Pratikte sorun değil: iki kanal aynı
...
- [ ] **Belge şablonları** — Dört tasarım (klasik/sade/kurumsal/kompakt),
      ...
      adım tek başına yayınlanabilir; ilk adım (şirket vurgu rengi) şablon
      altyapısını beklemeden görünür fark üretiyor.
